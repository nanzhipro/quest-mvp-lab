#!/bin/bash
# esmvp-swift 本机验证脚本（DoD 用例，对齐 ObjC 版 test.sh）。用法：sudo ./test.sh
...
T=/private/tmp/esmvp-swift-test
O=/private/tmp/esmvp-swift-other
...
"${BIN}" --stats-interval 3 > /tmp/esmvp-swift-case1.log 2>&1 &
P=$!; sleep 2
...
grep -E "mode=|stats" /tmp/esmvp-swift-case1.log
echo "期望：mode=mute-all，final 统计 received=0"
echo

echo "===== Case 2: watch 模式（--watch ${T}）====="
"${BIN}" --watch "${T}" --stats-interval 3 --verbose > /tmp/esmvp-swift-case2.log 2>&1 &
P=$!; sleep 2
...
echo "--- cat ${O}/x.png （目录外，期望正常打开且 esmvp-swift 无事件日志）:"; head -c4 "${O}/x.png" > /dev/null && echo "(opened ok)"
sleep 4; kill -TERM ${P} 2>/dev/null; wait ${P} 2>/dev/null
echo "--- esmvp-swift 日志:"; cat /tmp/esmvp-swift-case2.log
echo
echo "===== 判定 ====="

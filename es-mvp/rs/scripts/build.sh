#!/bin/bash
# esmvp-rs（Rust 版）构建签名：cargo release → 组 bundle → codesign → 校验。
#
...
cp target/release/esmvp-rs "$APP/Contents/MacOS/esmvp-rs"
...
echo "run: sudo ./$APP/Contents/MacOS/esmvp-rs [--watch <dir>]... [--cache] [--verbose]"

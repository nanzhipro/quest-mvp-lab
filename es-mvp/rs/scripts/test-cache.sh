#!/bin/bash
# esmvp-rs 内核授权缓存（--cache）语义验证，对齐 ObjC 版 test-cache.sh。用法: sudo ./test-cache.sh
# 原理：watch 目录隔离保证只有本脚本产生的 open 会被投递，事件计数即缓存行为的直接证据。
set -uo pipefail
cd "$(dirname "$0")/.."   # 项目根（es-mvp/rs/）

BIN=./ESMvpRs.app/Contents/MacOS/esmvp-rs
D=/private/tmp/esmvp-rs-cache
rm -rf "${D}"; mkdir -p "${D}"
echo v1 > "${D}/a.txt"
printf '\211PNG\r\n\032\n' > "${D}/deny.png"

EPID=""
start() { "${BIN}" --watch "${D}" --stats-interval 60 --verbose "$@" > /tmp/esmvp-rs-cache.log 2>&1 & EPID=$!; sleep 2; }
stop() { kill -TERM ${EPID} 2>/dev/null; wait ${EPID} 2>/dev/null; }
events() { grep -c 'decision=' /tmp/esmvp-rs-cache.log; }

#!/bin/bash
# esmvp（ObjC 版）构建签名：编译 → 组 bundle → codesign → 校验。
#
...
clang -fobjc-arc -O2 -Wall -mmacosx-version-min=13.0 \
  -framework Foundation -framework UniformTypeIdentifiers -lEndpointSecurity \
  src/main.m -o "$APP/Contents/MacOS/esmvp"
...
echo "run: sudo ./$APP/Contents/MacOS/esmvp [--watch <dir>]... [--cache] [--verbose]"

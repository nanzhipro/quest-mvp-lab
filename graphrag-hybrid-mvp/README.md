...
4. **社区通道基本没起作用，原因已定位**：14 个社区里最大一个吞掉了 188/292 个实体（**64%**），
   摘要因此退化成一张「值班、变更、事件、权限」的大杂烩清单，几乎每道题都会把它召回进来。
   这是标签传播在稠密小图上的已知行为（枢纽节点把标签染遍全图），
   也说明社区通道当前更像噪声源 —— 换 Leiden 的社区粒度、或对社区摘要也做相关性过滤，才谈得上收益。

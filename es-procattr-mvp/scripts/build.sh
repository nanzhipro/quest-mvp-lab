#!/bin/bash
# es-procattr-mvp 构建签名：swift release → 组 bundle → codesign → 校验。
#
...
# PROFILE 默认先看项目内，再看机器级 ~/.esmvp-local（本机历史 MVP 的 profile 安置处）
PROFILE="${PROFILE:-}"
if [ -z "$PROFILE" ]; then
  for cand in ./packaging/esmvp.provisionprofile "$HOME/.esmvp-local/esmvp_profile.provisionprofile"; do
...
if [ -z "$PROFILE" ] || [ ! -f "$PROFILE" ]; then
  ...
  echo "请按 README.md「开始前你需要准备」创建并下载，或放入 packaging/ 目录 / ~/.esmvp-local/" >&2
  exit 1
fi
...
echo "run: sudo ./$APP/Contents/MacOS/$BIN <config.json>   # 见 config.example.json"
